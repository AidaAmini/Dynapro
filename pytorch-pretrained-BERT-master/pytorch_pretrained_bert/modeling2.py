...
class Sequence_decoder(nn.Module):
	...
	def __init__(self, hidden_dim=300,  batch_size=8, n_class=4, sequence_length=200):
		super(Sequence_decoder, self).__init__()
		...
		self.before_state_start = None
		self.before_state_end = None
	...
	def forward(self, input_ids, 
				#token_type_ids=None, 
				# attention_mask=None,  start_positions=None, end_positions=None, answer_mask=None, known_switch=None, 
				# before_state_start_possitions=None, before_state_end_positions=None,before_state_answer_mask=None,before_state_known_switch=None,
				sequence_list , start_probs=None, end_probs=None, classification_probs=None, before_state_start_probs=None, before_state_end_probs=None, before_state_classification_probs=None, actions=None, unk_mask=None, none_mask=None, ans_mask=None, first_sentence=False):
		...
		if actions is not None:
			self.hidden = (Variable(self.hidden[0].data, requires_grad=True), Variable(self.hidden[1].data, requires_grad=True))
			if self.before_state_end is not None:
				self.before_state_start = Variable(self.before_state_start.data, requires_grad=True)
				self.before_state_end = Variable(self.before_state_end.data, requires_grad=True)
			

			weigthed_start_ans = torch.mul((start_probs * ans_mask), classification_probs[:,0].clone().unsqueeze(-1).expand(self.batch_size,200))
			weigthed_start_unk = torch.mul( unk_mask,classification_probs[:,1].clone().unsqueeze(-1).expand(self.batch_size,200))
			weigthed_start_none = torch.mul(none_mask ,classification_probs[:,2].clone().unsqueeze(-1).expand(self.batch_size,200))
			start_probs_with_attention = torch.mul(weigthed_start_unk.unsqueeze(2), sequence_list) +  torch.mul(weigthed_start_ans.unsqueeze(2), sequence_list) + torch.mul(weigthed_start_none.unsqueeze(2), sequence_list)

			weigthed_start_ans = torch.mul((end_probs * ans_mask) ,classification_probs[:,0].clone().unsqueeze(-1).expand(self.batch_size,200))
			weigthed_start_unk = torch.mul( unk_mask,classification_probs[:,1].clone().unsqueeze(-1).expand(self.batch_size,200))
			weigthed_start_none = torch.mul(none_mask ,classification_probs[:,2].clone().unsqueeze(-1).expand(self.batch_size,200))
			end_probs_with_attention = torch.mul(weigthed_start_unk.unsqueeze(2), sequence_list) +  torch.mul(weigthed_start_ans.unsqueeze(2), sequence_list) + torch.mul(weigthed_start_none.unsqueeze(2), sequence_list)
 
			if first_sentence == True:
				weigthed_start_ans = torch.mul((before_state_start_probs * ans_mask) ,before_state_classification_probs[:,0].clone().unsqueeze(-1).expand(self.batch_size,200))
				weigthed_start_unk = torch.mul( unk_mask,before_state_classification_probs[:,1].clone().unsqueeze(-1).expand(self.batch_size,200))
				weigthed_start_none = torch.mul(none_mask ,before_state_classification_probs[:,2].clone().unsqueeze(-1).expand(self.batch_size,200))
				before_state_start_probs_with_attention = torch.mul(weigthed_start_unk.unsqueeze(2), sequence_list) +  torch.mul(weigthed_start_ans.unsqueeze(2), sequence_list) + torch.mul(weigthed_start_none.unsqueeze(2), sequence_list)


				weigthed_start_ans = torch.mul((before_state_end_probs * ans_mask) ,before_state_classification_probs[:,0].clone().unsqueeze(-1).expand(self.batch_size,200))
				weigthed_start_unk = torch.mul( unk_mask,before_state_classification_probs[:,1].clone().unsqueeze(-1).expand(self.batch_size,200))
				weigthed_start_none = torch.mul(none_mask ,before_state_classification_probs[:,2].clone().unsqueeze(-1).expand(self.batch_size,200))
				before_state_end_probs_with_attention = torch.mul(weigthed_start_unk.unsqueeze(2), sequence_list) +  torch.mul(weigthed_start_ans.unsqueeze(2), sequence_list) + torch.mul(weigthed_start_none.unsqueeze(2), sequence_list)


				rnn_input = torch.cat((sequence_list, start_probs_with_attention, end_probs_with_attention, before_state_start_probs_with_attention, before_state_end_probs_with_attention), 2)

				# import pdb; pdb.set_trace()
			
			else:
				before_state_start_probs_with_attention = self.before_state_start
				before_state_end_probs_with_attention = self.before_state_end
				rnn_input = torch.cat((sequence_list, start_probs_with_attention, end_probs_with_attention, self.before_state_start, self.before_state_end), 2)
			

			# rnn_input = torch.cat((sequence_list, start_probs_with_attention, end_probs_with_attention, before_state_start_probs_with_attention, before_state_end_probs_with_attention), 2)

			# 	rnn_input = torch.cat((sequence_list, start_probs_with_attention, end_probs_with_attention), 2)
			# else:
			# 	rnn_input = torch.cat((sequence_list, sequence_list, sequence_list), 2)

			self.before_state_end = end_probs_with_attention
			self.before_state_start = start_probs_with_attention
			rnn_output, self.hidden = self.rnn(rnn_input, self.hidden)



			action_logits = self.non_linear(rnn_output)
			action_logits = self.action_classifier(action_logits.view(-1, 200*600))
			loss_fct = CrossEntropyLoss(reduce=False)
			# import pdb; pdb.set_trace()
			loss = torch.sum(loss_fct(action_logits, actions))
			return loss
		else:
			action_logits_list = []
...
class GlobalQuestionAnsweringWithLocationalPredictions_classes_only_all_in_one(PreTrainedBertModel):
	...
	def __init__(self, config, batch_size=8, n_class=3, hidden_dim=500, sequence_length=200):
		...
		########################################
		self.lstm_decoder_spans = nn.LSTM(2, self.hidden_dim,bidirectional=True,  batch_first=True).cuda()
		self.lstm_decoder_spans_before = nn.LSTM(2, self.hidden_dim,bidirectional=True,  batch_first=True).cuda()
		self.hidden_spans = self.init_hidden(self.batch_size)
		self.hidden_spans_before = self.init_hidden(self.batch_size)
		self.hidden_to_current_spans = nn.Linear(self.hidden_dim*2, 2)
		self.hidden_to_before_spans = nn.Linear(self.hidden_dim*2, 2)
	...
	def forward(self, input_ids, token_type_ids=None, attention_mask=None,  start_positions=None, end_positions=None, answer_mask=None, known_switch=None, before_state_start_positions=None, before_state_end_positions=None,before_state_answer_mask=None,before_state_known_switch=None):
		...
		if start_positions is not None and end_positions is not None: # training mode
			...
			self.hidden_start_before = (Variable(self.hidden_start_before[0].data, requires_grad=True), Variable(self.hidden_start_before[1].data, requires_grad=True))
			self.hidden_start = (Variable(self.hidden_start[0].data, requires_grad=True), Variable(self.hidden_start[1].data, requires_grad=True))
			self.hidden_spans = (Variable(self.hidden_spans[0].data, requires_grad=True), Variable(self.hidden_spans[1].data, requires_grad=True))
			...
			self.hidden_end_before = (Variable(self.hidden_end_before[0].data, requires_grad=True), Variable(self.hidden_end_beforen[1].data, requires_grad=True))
			self.hidden_end = (Variable(self.hidden_end[0].data, requires_grad=True), Variable(self.hidden_end[1].data, requires_grad=True))
			...
			# logits = self.qa_outputs(sequence_output)
			# current_logits, before_state_logits = logits.split(2, dim=-1)
			

			# # if i != 0:
			# #     current_logits = 0.1 * self.prev_logits + 0.9 * current_logits
			# # if i != 0:
			# #     before_state_logits = 0.1 * self.before_state_prev_logits + 0.9 * before_state_logits
			# self.prev_logits = current_logits.clone()
			# self.before_state_prev_logits = before_state_logits.clone()

			# current_logits, self.hidden_spans = self.lstm_decoder_spans(current_logits, self.hidden_spans)
			# before_logits, self.hidden_spans_before = self.lstm_decoder_spans(before_state_logits, self.hidden_spans_before)

			# current_logits = self.hidden_to_current_spans(current_logits)
			# start_logits, end_logits = current_logits.split(1, dim=-1)

			# before_logits = self.hidden_to_before_spans(before_logits)
			# before_state_start_logits, before_state_end_logits = before_logits.split(1, dim=-1)                
			...
			##########################################################################################
 # CUDA_VISIBLE_DEVICES=2 python3 global_local_with_decoder.py  --bert_model bert-base-uncased --do_train --do_predict   --do_lower_case   --train_file $SQUAD_DIR/train-gl-lc-v1.1.json   --predict_file $SQUAD_DIR/dev-gl-lc-v1.1.json   --train_batch_size 8 --predict_batch_size 8  --learning_rate 3e-5   --num_train_epochs 20.0   --max_seq_length 200   --doc_stride 128   --output_dir /tmp/debug_squad_no_lstm_encoding_batch8/
			...
			ignored_index = start_logits.size(1)
			start_positions.clone().clamp_(0, ignored_index)
			end_positions.clone().clamp_(0, ignored_index)
			...
			# import pdb; pdb.set_trace()
			return total_loss, start_probs, end_probs, switch_probs, before_state_start_probs, before_state_end_probs, before_state_switch_probs, sequence_output
			...
			return start_probs, end_probs, switch_probs, before_state_start_probs, before_state_end_probs, before_state_switch_probs, sequence_output





#before leakage problem




class Sequence_decoder_before(nn.Module):

	def __init__(self, hidden_dim=300,  batch_size=2, n_class=4, sequence_length=200):
		super(Sequence_decoder_before, self).__init__()
		self.batch_size = batch_size
		self.hidden_dim = hidden_dim
		self.rnn = nn.LSTM(3840, hidden_dim, bidirectional=True, batch_first=True)
		# self.qa_future_outputs = nn.Linear(config.hidden_size, 2)
	...
	def forward(self, input_ids, 
		...
		if actions is not None:
			...
			for i in range(len(input_ids[0])):
				...
				# import pdb; pdb.set_trace()
				weigthed_start_ans = torch.mul((start_probs[i] * ans_mask[:,i].clone()), classification_probs[i][:,0].clone().unsqueeze(-1).expand(2,200))
				weigthed_start_unk = torch.mul( unk_mask[:,i].clone(),classification_probs[i][:,1].clone().unsqueeze(-1).expand(2,200))
				weigthed_start_none = torch.mul(none_mask[:,i].clone() ,classification_probs[i][:,2].clone().unsqueeze(-1).expand(2,200))
				start_probs_with_attention = torch.mul(weigthed_start_unk.unsqueeze(2), sequence_list) +  torch.mul(weigthed_start_ans.unsqueeze(2), sequence_list) + torch.mul(weigthed_start_none.unsqueeze(2), sequence_list)

				weigthed_start_ans = torch.mul((end_probs[i] * ans_mask[:,i].clone()) ,classification_probs[i][:,0].clone().unsqueeze(-1).expand(2,200))
				weigthed_start_unk = torch.mul( unk_mask[:,i].clone(),classification_probs[i][:,1].clone().unsqueeze(-1).expand(2,200))
				weigthed_start_none = torch.mul(none_mask[:,i].clone() ,classification_probs[i][:,2].clone().unsqueeze(-1).expand(2,200))
				end_probs_with_attention = torch.mul(weigthed_start_unk.unsqueeze(2), sequence_list) +  torch.mul(weigthed_start_ans.unsqueeze(2), sequence_list) + torch.mul(weigthed_start_none.unsqueeze(2), sequence_list)

				if i == 0:
					weigthed_start_ans = torch.mul((before_state_start_probs[i] * ans_mask[:,i].clone()) ,before_state_classification_probs[i][:,0].clone().unsqueeze(-1).expand(2,200))
					weigthed_start_unk = torch.mul( unk_mask[:,i].clone(),before_state_classification_probs[i][:,1].clone().unsqueeze(-1).expand(2,200))
					weigthed_start_none = torch.mul(none_mask[:,i].clone() ,before_state_classification_probs[i][:,2].clone().unsqueeze(-1).expand(2,200))
					before_state_start_probs_with_attention = torch.mul(weigthed_start_unk.unsqueeze(2), sequence_list) +  torch.mul(weigthed_start_ans.unsqueeze(2), sequence_list) + torch.mul(weigthed_start_none.unsqueeze(2), sequence_list)


					weigthed_start_ans = torch.mul((before_state_end_probs[i] * ans_mask[:,i].clone()) ,before_state_classification_probs[i][:,0].clone().unsqueeze(-1).expand(2,200))
					weigthed_start_unk = torch.mul( unk_mask[:,i].clone(),before_state_classification_probs[i][:,1].clone().unsqueeze(-1).expand(2,200))
					weigthed_start_none = torch.mul(none_mask[:,i].clone() ,before_state_classification_probs[i][:,2].clone().unsqueeze(-1).expand(2,200))
					before_state_end_probs_with_attention = torch.mul(weigthed_start_unk.unsqueeze(2), sequence_list) +  torch.mul(weigthed_start_ans.unsqueeze(2), sequence_list) + torch.mul(weigthed_start_none.unsqueeze(2), sequence_list)
					...
					rnn_input = torch.cat((sequence_list, start_probs_with_attention, end_probs_with_attention, before_state_start, before_state_end), 2)
			...
			for i in range(len(input_ids[0])):
				if sum(input_ids[0][i]).cpu().numpy() == 0:
					action_logits_list.append(torch.zeros(4))
					continue
...
class GlobalQuestionAnsweringWithLocationalPredictions_classes_only_all_in_one_before(PreTrainedBertModel):

	def __init__(self, config, n_class=3, hidden_dim=1000, sequence_length=200):
		super(GlobalQuestionAnsweringWithLocationalPredictions_classes_only_all_in_one_before, self).__init__(config)
		self.batch_size = 2
		self.bert = BertModel(config)
		# TODO check with Google if it's normal there is no dropout on the token classifier of SQuAD in the TF version
		# self.word_embeddings = nn.Embedding(vocab_size, embedding_dim)
	...
	def detach(self):
		self.hidden.detach_()
		self.hidden_start_before.detach_()
		self.hidden_start.detach_()
		self.hidden_spans.detach_()
		self.hidden_spans_before.detach_()
	...
	def forward(self, input_ids, token_type_ids=None, attention_mask=None,  start_positions=None, end_positions=None, answer_mask=None, known_switch=None, before_state_start_positions=None, before_state_end_positions=None,before_state_answer_mask=None,before_state_known_switch=None):
		...
		if start_positions is not None and end_positions is not None: # training mode
			...
			for i in range(len(input_ids[0])):
				...
				end_logits_list.append(end_probs)
				# switch_logits_list.append(switch_probs)
				switch_logits_list.append(torch.ones(1,3).cuda())
				before_state_start_logits_list.append(before_state_start_probs)
				before_state_end_logits_list.append(before_state_end_probs)
				# before_state_switch_logits_list.append(before_state_switch_probs)
				before_state_switch_logits_list.append(torch.ones(1,3).cuda())
...
class GlobalQuestionAnsweringWithLocationalPredictions_classes_only_all_in_one_before_ablation2(PreTrainedBertModel):

	def __init__(self, config, n_class=3, hidden_dim=1000, sequence_length=200):
		super(GlobalQuestionAnsweringWithLocationalPredictions_classes_only_all_in_one_before_ablation2, self).__init__(config)
		self.batch_size = 2
		self.bert = BertModel(config)
		# TODO check with Google if it's normal there is no dropout on the token classifier of SQuAD in the TF version
		# self.word_embeddings = nn.Embedding(vocab_size, embedding_dim)
...
class GlobalQuestionAnsweringWithLocationalPredictions_classes_only_all_in_one_before_ablation1(PreTrainedBertModel):
...
class GlobalQuestionAnsweringWithLocationalPredictions_classes_only_all_in_one1(PreTrainedBertModel):

	def __init__(self, config, n_class=3, hidden_dim=500, sequence_length=200):
		super(GlobalQuestionAnsweringWithLocationalPredictions_classes_only_all_in_one1, self).__init__(config)
		self.bert = BertModel(config)
		# TODO check with Google if it's normal there is no dropout on the token classifier of SQuAD in the TF version
		# self.word_embeddings = nn.Embedding(vocab_size, embedding_dim)
		self.hidden_dim = hidden_dim
		...
		self.qa_classifier = nn.Linear(config.hidden_size*sequence_length, n_class)
		self.qa_classifier_before = nn.Linear(config.hidden_size*sequence_length, n_class)
		self.hidden = self.init_hidden1(2)
