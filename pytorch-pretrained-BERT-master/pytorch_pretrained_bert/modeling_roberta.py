...
class GlobalQuestionAnsweringWithLocationalPredictions_classes_only_all_in_one(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,  start_positions=None, end_positions=None, answer_mask=None, known_switch=None, 
        ...
        if start_positions is not None and end_positions is not None: # training mode
            ...
            for i in range(len(input_ids[0])):
                ...
                sequence_output, _ = self.roberta(input_ids[:,i].clone(), token_type_ids[:,i].clone(), attention_mask[:,i].clone(), output_all_encoded_layers=False)
