...
####################################################
class GlobalQuestionAnsweringWithLocationalPredictions(BertPreTrainedModel):

    def __init__(self, config, n_class=3, hidden_dim=1000, sequence_length=200):
        super(GlobalQuestionAnsweringWithLocationalPredictions, self).__init__(config)
        self.batch_size = 8
        ...
        self.lstm_for_tagging = nn.LSTM(config.hidden_size, self.hidden_dim, bidirectional=True,  batch_first=True).cuda()
...
#########################################################################################################################
class GlobalQuestionAnsweringWithLocationalPredictions_classes_only_all_in_one_cooking_loc_no_before(BertPreTrainedModel):

    def __init__(self, config, n_class=244, hidden_dim=1000, sequence_length=350):
        super(GlobalQuestionAnsweringWithLocationalPredictions_classes_only_all_in_one_cooking_loc_no_before, self).__init__(config)
        self.batch_size = 8
        self.hidden_dim = hidden_dim
        self.sequence_length = sequence_length
        self.bert_hidden_size = config.hidden_size
...
class Sequence_decoder_simple(action_decoder):
    ...
    def __init__(self, hidden_dim=300, input_dim=768,  batch_size=8, n_class=4, sequence_length=200):
        ...
        self.sequence_length = sequence_length
        self.bert_hidden_size = input_dim
        self.action_classifier = nn.Linear(input_dim * self.sequence_length, n_class)
    ...
    def forward(self, input_ids, sequence_list , start_probs=None, end_probs=None, classification_probs=None, 
        ...
        if actions is not None:  #training
            ...
            for i in range(len(input_ids[0])):
                ...
                action_logits = self.action_classifier(sequence_list[i].contiguous().view(-1, self.sequence_length*self.bert_hidden_size))
